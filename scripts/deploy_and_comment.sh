...
# Cloud Build から渡される環境変数:
# PROJECT_ID, COMMIT_SHA, _REGION, _AR_REPO, _SERVICE, _WORKFLOW, _GH_OWNER, _GH_REPO, GITHUB_TOKEN
...
curl -sS -X POST \
  ...
  "https://api.github.com/repos/${_GH_OWNER}/${_GH_REPO}/commits/${COMMIT_SHA}/comments" \
  -d "${PAYLOAD}" | cat

echo "Posted deploy URL to commit comment."#!/usr/bin/env bash
set -euo pipefail
